...
// The String methods for this package's enum-like types are all generated by
// the single stringer run below; type-checking this large package costs far
// more than stringer's own work, so a run per type would repeat it needlessly.
// Note that -trimprefix and -linecomment apply to every type listed.
//
//go:generate go tool stringer -type=ArcType,vertexStatus,ErrorCode,Op,runMode,defIDType -trimprefix=Arc -linecomment -output=stringer_gen.go
