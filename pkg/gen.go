...
type emitter interface {
	...
	// maintained alongside its Go sources, as a single expression
	// wrapping the file's declarations.
	supplementalCUE(expr ast.Expr) error
...
func (es emitters) supplementalCUE(expr ast.Expr) error {
	for _, e := range es {
		if err := e.supplementalCUE(expr); err != nil {
			return err
		}
	}
	return nil
...
// generate writes the generated files for a package.
func generate(pkg *packages.Package) error {
	...
	if !skipRegister {
		...
		expr, err := loadSupplementalCUE(pkgDir)
		if err != nil {
			return fmt.Errorf("error processing %s: %v", cuePkg, err)
		}
		if expr != nil {
			if err := emits.supplementalCUE(expr); err != nil {
				return err
			}
		}
	}
...
// If the directory has no CUE file, it returns (nil, nil).
//
...
func loadSupplementalCUE(dir string) (ast.Expr, error) {
...
// builtins.
func (g *goEmitter) supplementalCUE(expr ast.Expr) error {
